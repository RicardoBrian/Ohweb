...
const PURPOSE_TEXT = {
  ...
  edu: '이 앱은 교육 현장(수업, 평가, 기록)에서 사용됩니다. 학생 개인정보는 서버로 전송하거나 외부에 저장하지 말고, 필요하면 브라우저 로컬에만 저장해주세요. 인쇄하거나 캡처해서 보관하기 좋은 레이아웃을 고려해주세요.',
...
const CLOSING_TEXT = '빌드 도구(Vite, React, webpack 등) 없이 순수 HTML/CSS/JS 단일 파일로 작성해주세요. 데이터 저장이 필요하면 브라우저 localStorage를 사용해주세요(Firebase나 다른 외부 데이터베이스는 쓰지 마세요). 완성된 웹앱을 별도 설명 없이 바로 실행 가능한 코드로 한 번에 작성해주세요.';
